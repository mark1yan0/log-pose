import type { IData, IMarker, IPlace, IPlaceOptions } from '$lib/types/data';
...
class PlacesManager {
	...
	public delete(place: IPlace) {
		const idx = this.places.findIndex((p) => p.data.place_id === place.data.place_id);
		...
		const featureByContry = this.geojson.shapes.map((f) => f.properties.name);
		for (const c of featureByContry) {
			if (!curr.includes(c)) {
				// TODO: handle
				const feature = this.geojson.shapes.find((s) => s.properties.name === c);
				// TODO: solve issue where is removed only on reload
				this.geojson.instance?.removeLayer(feature);
				this.geojson.shapes = this.geojson.shapes.filter((f) => f.properties.name !== c);
			}
		}
	...
	private updateGeoJSON(place: IPlace, config?: IPlaceOptions) {
		if (
			...
			this.geojson.shapes.find((f) => f.properties.name === place.data.address.country)
		) {
			// is already in there. skip
			return;
		}

		const found = countries.features.find((f) => f.properties.name === place.data.address.country);
