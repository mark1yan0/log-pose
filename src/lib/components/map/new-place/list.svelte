<script lang="ts">
	...
	import NewPlaceForm from './new-place-form.svelte';
	...
	let selected = $state<IPlace | null>(null);
...
<Command.Dialog bind:open shouldFilter={false}>
	...
	{#if selected}
		<Command.Group>
			<button onclick={() => selectionCloseHandler({ close: false })}><ChevronLeft /></button>
			<br />
			<NewPlaceForm {clearQuery} place={selected} onSelectionClose={selectionCloseHandler} />
		</Command.Group>
	{:else}
		{#if !!query.error}
			<Command.Item>
		...
		{#if query.isLoading}
			<Command.Loading class="p-2">
				<Skeleton class="mb-2 h-16 w-full" />
				<Skeleton class="mb-2 h-16 w-full" />
				<Skeleton class="mb-2 h-16 w-full" />
...
<Button
	variant="secondary"
	command={OPEN_KEY}
	class="over-map right-2 bottom-6"
	onclick={() => {
		open = !open;
	}}
>
