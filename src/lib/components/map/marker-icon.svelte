<script lang="ts">
	...
	import type { TMarkerIcon } from '$lib/types/data';
	import { DivIcon } from 'sveaflet';

	const { marker, contained }: { marker: TMarkerIcon; contained?: boolean } = $props();
...
{#if contained}
	<DivIcon
		options={{
			className: 'bg-primary border-none !w-fit !h-fit p-2 rounded-full !ml-[-1rem] !mt-[-1rem]'
		}}
	>
		{#if marker === 'pin'}
